...
var Scope = function() {
  ...
  return ($traceurRuntime.createClass)(Scope, {
    ...
    addDeclaration: function(tree, type, reporter) {
      var name = tree.getStringValue();
      if ((this.lexicalDeclarations_.has(name) || this.variableDeclarations_.has(name)) && !this.isFunctionName(name)) {
        reportDuplicateVar(reporter, tree, name);
        return;
      }
    ...
    isFunctionName: function(name) {
      var varScope = this.getVarScope();
      return varScope && varScope.tree.type === FUNCTION_EXPRESSION && varScope.tree.name.getStringValue() === name;
    },
